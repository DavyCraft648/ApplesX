...
class GoldOreApple extends \pocketmine\item\Apple implements \customiesdevs\customies\item\ItemComponents{
	use ItemComponentsTrait;

	public function __construct(ItemIdentifier $identifier, string $name = "Unknown"){
		parent::__construct($identifier, $name);
		$this->initComponent("gold_ore_apple", new CreativeInventoryInfo(CreativeInventoryInfo::CATEGORY_NATURE, CreativeInventoryInfo::GROUP_CROP));
				...
				for($z = ($pos->z - 1); $z <= ($pos->z + 1); $z++){
					for($y = ($pos->y - 3); $y <= $pos->y; $y++){
						if($world->getBlockAt($x, $y, $z)->isSameType(VanillaBlocks::STONE())){
							$world->setBlockAt($x, $y, $z, VanillaBlocks::GOLD_ORE());
						}
					}
				}
			}
